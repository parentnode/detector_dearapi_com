...
* This file contains country maintenance functionality
*/
include_once("country.view.class.php");
...
* Country
*/
class Country extends CountryView {
...
//		$this->translater = new Translation(__FILE__);
		...
		$this->file = FRAMEWORK_PATH."/library/basics/countries.xml";

		$this->db = UT_BAS_COU;

		$this->varnames["name"] = $this->translate("Country");

		$this->varnames["id"] = $this->translate("ISO-2");
		$this->validator->rule("id", "txt", false, 2, 2);
		$this->validator->rule("id", "unik", $this->translate("Country exists!"), $this->db);
	...
	*/
	function saveItem($id = false) {
		if($this->validator->validateAll()) {
			...
			$countries = $this->getAllItems();
			$name = $countries["values"][array_search($id, $countries["id"])];
