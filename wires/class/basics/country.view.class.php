...
* This file contains Country views functionality
* Extended by the actions class
*/
class CountryView extends Translation {
	...
	*/
	function viewItem() {
		...
		$_ = '';
		$_ .= $HTML->head($this->translate("Country").": ".$this->getQueryResult(0, "name"));
	...
	*/
	function newItem() {
		...
		$_ = '';
		$_ .= $HTML->head($this->translate("New country"));
	...
	*/
	function listItems($link=false, $validate=false) {
		...
		$_ = $HTML->head($this->translate("Countries"));
		$_ .= Generic::listItemsExtended($link, $validate, $items["id"], array($items["values"], $items["id"]), array($this->translate("Country name"), $this->translate("search")), array("max", "search acenter"));
